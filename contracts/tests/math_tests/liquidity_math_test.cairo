...
fn test_swap_calculation() {
    ...
    assert(
        price_next.value >= expected_price_next
            - 100 && price_next.value <= expected_price_next
            + 100,
        'Price calculation incorrect',
    );

    // Calculate amounts
    let amount_in_calculated = LiquidityMath::calc_amount1_delta(
        current_sqrtp.clone(), price_next.clone(), liquidity,
    );
    let amount_out_calculated = LiquidityMath::calc_amount0_delta(
        current_sqrtp, price_next, liquidity,
    );
    ...
    assert(
        amount_in_calculated >= expected_amount_in
            - tolerance_in && amount_in_calculated <= expected_amount_in
            + tolerance_in,
        'Amount in incorrect',
    );
    assert(
        amount_out_calculated >= expected_amount_out
            - tolerance_out && amount_out_calculated <= expected_amount_out
            + tolerance_out,
        'Amount out incorrect',
    );
}
